...
TEST(
  ...
  EXPECT_EQ(uint64_t(ZE_RESULT_ERROR_INVALID_NULL_HANDLE),
            uint64_t(zeModuleCreate(nullptr, nullptr, nullptr, nullptr)));
}
TEST(
    ModuleCreateNegativeTests,
  ...
  EXPECT_EQ(uint64_t(ZE_RESULT_ERROR_INVALID_NULL_POINTER),
            uint64_t(zeModuleCreate(device, &module_description, nullptr,
...
TEST(
  ...
  EXPECT_EQ(
      ...
      uint64_t(zeModuleCreate(device, &module_description, &module, nullptr)));
}
TEST(
