...
uint32_t get_ras_handles_count(zes_device_handle_t device) {
  uint32_t count = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesDeviceEnumRasErrorSets(device, &count, nullptr));
  EXPECT_GE(count, 0);
  return count;
}
...
zes_ras_properties_t get_ras_properties(zes_ras_handle_t rasHandle) {
  zes_ras_properties_t properties = {ZES_STRUCTURE_TYPE_RAS_PROPERTIES,
                                     nullptr};
  EXPECT_ZE_RESULT_SUCCESS(zesRasGetProperties(rasHandle, &properties));
  return properties;
}
...
zes_ras_config_t get_ras_config(zes_ras_handle_t rasHandle) {
  ...
  EXPECT_ZE_RESULT_SUCCESS(zesRasGetConfig(rasHandle, &rasConfig));
  return rasConfig;
}

void set_ras_config(zes_ras_handle_t rasHandle, zes_ras_config_t rasConfig) {
  EXPECT_ZE_RESULT_SUCCESS(zesRasSetConfig(rasHandle, &rasConfig));
}
...
zes_ras_state_t get_ras_state(zes_ras_handle_t rasHandle, ze_bool_t clear) {
  zes_ras_state_t state = {ZES_STRUCTURE_TYPE_RAS_STATE, nullptr};
  EXPECT_ZE_RESULT_SUCCESS(zesRasGetState(rasHandle, clear, &state));
  return state;
}
std::vector<zes_ras_state_exp_t>
ras_get_state_exp(zes_ras_handle_t ras_handle) {
  uint32_t count = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesRasGetStateExp(ras_handle, &count, NULL));
  EXPECT_GT(count, 0);
...
void ras_clear_state_exp(zes_ras_handle_t ras_handle,
  ...
  EXPECT_ZE_RESULT_SUCCESS(zesRasClearStateExp(ras_handle, category));
}
} // namespace level_zero_tests
