...
std::vector<zes_temp_handle_t> get_temp_handles(ze_device_handle_t device,
  ...
  EXPECT_ZE_RESULT_SUCCESS(
      zesDeviceEnumTemperatureSensors(device, &count, temp_handles.data()));
  return temp_handles;
}
...
double get_temp_state(zes_temp_handle_t tempHandle) {
  double temp = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesTemperatureGetState(tempHandle, &temp));
  return temp;
}
...
zes_temp_config_t get_temp_config(zes_temp_handle_t tempHandle) {
  ...
  EXPECT_ZE_RESULT_SUCCESS(zesTemperatureGetConfig(tempHandle, &config));
  return config;
}
