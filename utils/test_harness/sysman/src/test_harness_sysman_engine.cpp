...
uint32_t get_engine_handle_count(zes_device_handle_t device) {
  uint32_t count = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesDeviceEnumEngineGroups(device, &count, nullptr));
  EXPECT_GE(count, 0);
  return count;
}
...
zes_engine_properties_t get_engine_properties(zes_engine_handle_t hEngine) {
  zes_engine_properties_t properties = {ZES_STRUCTURE_TYPE_ENGINE_PROPERTIES,
                                        nullptr};
  EXPECT_ZE_RESULT_SUCCESS(zesEngineGetProperties(hEngine, &properties));
  return properties;
}

zes_engine_stats_t get_engine_activity(zes_engine_handle_t hEngine) {
  zes_engine_stats_t stats = {};
  EXPECT_ZE_RESULT_SUCCESS(zesEngineGetActivity(hEngine, &stats));
  return stats;
}
