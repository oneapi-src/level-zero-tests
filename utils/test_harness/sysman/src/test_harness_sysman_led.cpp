...
uint32_t get_led_handle_count(zes_device_handle_t device) {
  uint32_t count = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesDeviceEnumLeds(device, &count, nullptr));
  EXPECT_GE(count, 0);
  return count;
}
