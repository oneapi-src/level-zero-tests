...
zes_oc_mode_t get_oc_mode(zes_freq_handle_t freqHandle) {
  zes_oc_mode_t mode = {};
  EXPECT_ZE_RESULT_SUCCESS(zesFrequencyOcGetMode(freqHandle, &mode));
  return mode;
}

void set_oc_mode(zes_freq_handle_t freqHandle, zes_oc_mode_t mode) {
  EXPECT_ZE_RESULT_SUCCESS(zesFrequencyOcSetMode(freqHandle, mode));
}

double get_oc_iccmax(zes_freq_handle_t freqHandle) {
  double icmax = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesFrequencyOcGetIccMax(freqHandle, &icmax));
  return icmax;
}
void set_oc_iccmax(zes_freq_handle_t freqHandle, double icmax) {
  EXPECT_ZE_RESULT_SUCCESS(zesFrequencyOcSetIccMax(freqHandle, icmax));
}
double get_oc_tjmax(zes_freq_handle_t freqHandle) {
  double tjmax = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesFrequencyOcGetTjMax(freqHandle, &tjmax));
  return tjmax;
}
void set_oc_tjmax(zes_freq_handle_t freqHandle, double tjmax) {
  EXPECT_ZE_RESULT_SUCCESS(zesFrequencyOcSetTjMax(freqHandle, tjmax));
}
