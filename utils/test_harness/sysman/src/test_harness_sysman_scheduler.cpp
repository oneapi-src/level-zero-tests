...
uint32_t get_scheduler_handle_count(zes_device_handle_t device) {
  uint32_t p_count = 0;
  EXPECT_ZE_RESULT_SUCCESS(zesDeviceEnumSchedulers(device, &p_count, nullptr));
  EXPECT_GE(p_count, 0);
  return p_count;
}
...
zes_sched_mode_t get_scheduler_current_mode(zes_sched_handle_t hScheduler) {
  ...
  EXPECT_ZE_RESULT_SUCCESS(zesSchedulerGetCurrentMode(hScheduler, &cur_mode));
  return cur_mode;
}
...
get_timeout_properties(zes_sched_handle_t hScheduler, ze_bool_t pDefault) {
  ...
  EXPECT_ZE_RESULT_SUCCESS(
      zesSchedulerGetTimeoutModeProperties(hScheduler, pDefault, &properties));
  return properties;
}
...
get_timeslice_properties(zes_sched_handle_t hScheduler, ze_bool_t pDefault) {
  ...
  EXPECT_ZE_RESULT_SUCCESS(zesSchedulerGetTimesliceModeProperties(
...
void set_exclusive_mode(zes_sched_handle_t hScheduler) {
  ze_bool_t reload = {};
  EXPECT_ZE_RESULT_SUCCESS(zesSchedulerSetExclusiveMode(hScheduler, &reload));
}
...
zes_sched_properties_t get_scheduler_properties(zes_sched_handle_t hScheduler) {
  zes_sched_properties_t properties = {ZES_STRUCTURE_TYPE_SCHED_PROPERTIES,
                                       nullptr};
  EXPECT_ZE_RESULT_SUCCESS(zesSchedulerGetProperties(hScheduler, &properties));
  return properties;
}
} // namespace level_zero_tests
