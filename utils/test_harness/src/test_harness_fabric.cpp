...
#include "test_harness/test_harness_fabric.hpp"
...
uint32_t get_ze_fabric_vertex_count(ze_driver_handle_t driver) {
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeFabricVertexGetExp(driver, &count, nullptr));
...
get_ze_fabric_sub_vertices(ze_fabric_vertex_handle_t vertex, uint32_t count) {
  ...
  auto vertex_initial = vertex;
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeFabricVertexGetSubVerticesExp(
                                   vertex, &count, sub_vertices.data()));
  EXPECT_EQ(vertex, vertex_initial);
  return sub_vertices;
}
...
get_ze_fabric_vertex_properties(ze_fabric_vertex_handle_t vertex) {
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS,
            zeFabricVertexGetPropertiesExp(vertex, &properties));
  EXPECT_EQ(vertex, vertex_initial);
  return properties;
}
...
get_ze_fabric_edge_properties(ze_fabric_edge_handle_t edge) {
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeFabricEdgeGetPropertiesExp(edge, &properties));
