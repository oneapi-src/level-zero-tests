...
get_device_metric_groups_by_source_id(
  ...
  // Query each handle's source ID exactly once and group the handles by it.
  // Using std::map keeps source IDs in ascending order for deterministic
  // iteration.
  std::map<uint32_t, std::vector<zet_metric_group_handle_t>>
      metric_groups_by_source;
...
bool optimize_metric_group_info_list(
  ...
  // Group all handles by their source ID in a single pass over the device
  std::map<uint32_t, std::vector<zet_metric_group_handle_t>>
      metricGroupsBySource =
  ...
  for (auto const &sourceEntry : metricGroupsBySource) {
    ...
    std::transform(MetricGroupHandlePerSource.begin(),
                   MetricGroupHandlePerSource.begin() + sourceCnt,
...
ze_result_t commandlist_append_streamer_marker(
    ...
    zet_metric_streamer_handle_t metricStreamerHandle, uint32_t marker) {
  return zetCommandListAppendMetricStreamerMarker(commandList,
                                                  metricStreamerHandle, marker);
}
...
void generate_device_list_with_activatable_metric_group_handles(
  ...
  for (auto device : devices) {
    ...
    std::vector<lzt::metricGroupInfo_t> metric_group_info_optimized;
    const bool optimized = lzt::optimize_metric_group_info_list(
        metric_group_info, 20, nullptr, 1, metric_group_info_optimized);
    if (!optimized) {
      continue;
    }
    metric_group_info = metric_group_info_optimized;
...
void run_matrix_multiplication_and_collect_query_data(
  ...
  zet_metric_query_pool_handle_t query_pool =
      lzt::create_metric_query_pool_for_device(
          device, 1, ZET_METRIC_QUERY_POOL_TYPE_PERFORMANCE, metric_group);
  zet_metric_query_handle_t query = lzt::metric_query_create(query_pool, 0);

  lzt::append_metric_query_begin(command_list, query);
  ...
  lzt::append_metric_query_end(command_list, query, nullptr);
  ...
  lzt::metric_query_get_data(query, &raw_data);

  lzt::destroy_metric_query(query);
  lzt::destroy_metric_query_pool(query_pool);
  lzt::destroy_function(function);
  lzt::destroy_module(module);
  lzt::free_memory(a_buffer);
  lzt::free_memory(b_buffer);
