...
protected:
  ...
  void dispatch_kernels(
    ...
    [[maybe_unused]] std::vector<Buffer> host_dst_ptrs(number_of_dispatch);
    [[maybe_unused]] kernel_copy_unit_t *src_allocation_ptrs = nullptr;
    allocate_memory<kernel_copy_unit_t>(
        context, device, test_arguments_.memory_type,
        number_of_dispatch * sizeof(void *), false);
    [[maybe_unused]] kernel_copy_unit_t *dst_allocation_ptrs = nullptr;
    allocate_memory<kernel_copy_unit_t>(
        context, device, test_arguments_.memory_type,
        number_of_dispatch * sizeof(void *), false);

    if (test_arguments_.indirect_access) {
      src_allocation_ptrs = allocate_memory<kernel_copy_unit_t>(
          context, device, test_arguments_.memory_type,
          number_of_dispatch * sizeof(void *), false);
      dst_allocation_ptrs = allocate_memory<kernel_copy_unit_t>(
          context, device, test_arguments_.memory_type,
          number_of_dispatch * sizeof(void *), false);
      for (uint32_t i = 0; i < number_of_dispatch; i++) {
        host_src_ptrs[i].data = src_allocations[i];
        host_dst_ptrs[i].data = dst_allocations[i];
      }
      ...
      if (test_arguments_.memory_type == ZE_MEMORY_TYPE_DEVICE) {
        ...
        std::memcpy(dst_allocation_ptrs, host_dst_ptrs.data(),
                    number_of_dispatch * sizeof(void *));
      }
    }
    ...
    for (uint64_t dispatch_id = 0; dispatch_id < number_of_dispatch;
      ...
      if (test_arguments_.indirect_access) {
        switch (test_arguments_.memory_type) {
        case ZE_MEMORY_TYPE_DEVICE:
          lzt::kernel_set_indirect_access(
              kernel_handle, ZE_KERNEL_INDIRECT_ACCESS_FLAG_DEVICE);
          break;
        case ZE_MEMORY_TYPE_HOST:
          lzt::kernel_set_indirect_access(kernel_handle,
                                          ZE_KERNEL_INDIRECT_ACCESS_FLAG_HOST);
          break;
        case ZE_MEMORY_TYPE_SHARED:
          lzt::kernel_set_indirect_access(
              kernel_handle, ZE_KERNEL_INDIRECT_ACCESS_FLAG_SHARED);
          break;
        default:
          break;
        }
        lzt::set_argument_value(kernel_handle, 0, sizeof(src_allocation_ptrs),
                                &src_allocation_ptrs);
        lzt::set_argument_value(kernel_handle, 1, sizeof(dst_allocation_ptrs),
                                &dst_allocation_ptrs);
        lzt::set_argument_value(kernel_handle, 2, sizeof(uint32_t),
                                &dispatch_id);
      } else {
...
LZT_TEST_P(
  ...
  test_arguments_.print_test_arguments(device_properties);
  ...
  const uint32_t used_vectors_in_test =
      test_arguments_.memory_type == ZE_MEMORY_TYPE_DEVICE ? 4 : 3;
  uint32_t number_of_dispatches = to_u32(test_arguments_.multiplier);
  ...
  adjust_max_memory_allocation(
      ...
      number_of_all_allocations, test_arguments_, relax_memory_capability);
  ...
  for (uint32_t dispatch_id = 0; dispatch_id < number_of_dispatches;
    ...
    kernel_name_ss << "test_device_memory" +
                          std::to_string(
                              (dispatch_id % number_of_kernels_in_module_) + 1);
    if (test_arguments_.indirect_access) {
      kernel_name_ss << "_indirect";
    } else {
      kernel_name_ss << "_unit_size" << std::to_string(kernel_copy_unit_size);
    }
    test_kernel_names.push_back(kernel_name_ss.str());
  }
