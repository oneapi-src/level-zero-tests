...
kernel void debug_add_constant(global int *values, int addval) {
...
kernel void debug_add_constant_2(global uchar *values, int addval) {
...
kernel void debug_add_two_arrays(global int *output, global int *input) {
  const int tid = get_global_id(0);
  output[tid] = output[tid] + input[tid];
}

kernel __attribute__((work_group_size_hint(1, 1, 1))) void
debug_add_attr(global int *output, global int *input) {
  const int tid = get_global_id(0);
  output[tid] = output[tid] + input[tid];
}
