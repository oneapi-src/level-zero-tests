...
void run_test(const ze_device_handle_t &device,
  ...
  auto metricGroupInfo = lzt::get_device_metric_groups_for_sampling_type(
      device, ZET_METRIC_GROUP_SAMPLING_TYPE_FLAG_EVENT_BASED, true);
  ASSERT_GT(metricGroupInfo.size(), 0u) << "No query metric groups found";
  metricGroupInfo = lzt::optimize_metric_group_info_list(
      metricGroupInfo, reset ? 1 : 20, nullptr, 1);
...
void run_multi_device_query_load_test(
  ...
  metric_group_info_0 =
      lzt::optimize_metric_group_info_list(metric_group_info_0, 20, nullptr, 1);
  metric_group_info_1 =
      lzt::optimize_metric_group_info_list(metric_group_info_1, 20, nullptr, 1);
...
LZT_TEST_F(
  ...
  for (auto device : devices) {
    ...
    ASSERT_GT(metricGroupInfo.size(), 0u) << "No query metric groups found";
    metricGroupInfo =
        lzt::optimize_metric_group_info_list(metricGroupInfo, 1, nullptr, 1);
