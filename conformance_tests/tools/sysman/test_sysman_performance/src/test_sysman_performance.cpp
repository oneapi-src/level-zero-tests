...
EXPECT_LE(properties.engines, ZES_ENGINE_TYPE_FLAG_OTHER | ZES_ENGINE_TYPE_FLAG_COMPUTE | ZES_ENGINE_TYPE_FLAG_3D | ZES_ENGINE_TYPE_FLAG_MEDIA | ZES_ENGINE_TYPE_FLAG_DMA);
    }
  }
}
...
for (auto device : devices) {
 ...
 double factor = 50;      // Checking with a random factor value within its range
