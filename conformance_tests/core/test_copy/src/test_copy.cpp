...
protected:
  void RunGivenMemAdviseWhenWritingFromDeviceThenDataIsCorrectFromHostTest(
      ze_memory_advice_t mem_advice, bool is_immediate, bool is_shared_system);
};

void zeCommandListAppendMemAdviseTests::
    RunGivenMemAdviseWhenWritingFromDeviceThenDataIsCorrectFromHostTest(
        ze_memory_advice_t mem_advice, bool is_immediate,
        bool is_shared_system) {
  const size_t size = 16;
  const uint8_t value = 0x55;
  void *memory =
