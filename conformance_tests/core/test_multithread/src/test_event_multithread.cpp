...
void ThreadEventSync(const ze_command_queue_handle_t cmd_queue) {
  ...
  for (uint32_t i = 0; i < num_iterations; i++) {
    ...
    lzt::reset_command_list(cmd_list[0]);
    ...
    lzt::destroy_event(events[0]);
    lzt::destroy_command_list(cmd_list[0]);

    // As per the spec, index could be re-used after destroy
    events[0] = lzt::create_event(event_pool, eventDesc[0]);
    for (auto event : events) {
      EXPECT_EQ(ZE_RESULT_NOT_READY, zeEventQueryStatus(event));
    }
    cmd_list[0] = lzt::create_command_list();
...
void ThreadMultipleEventsSync(const ze_command_queue_handle_t cmd_queue) {
  ...
  for (uint32_t i = 0; i < num_iterations; i++) {
    ...
    lzt::destroy_command_list(cmd_list[0]);
    lzt::destroy_command_list(cmd_list[1]);
    ...
    cmd_list[0] = lzt::create_command_list();
    cmd_list[1] = lzt::create_command_list();
