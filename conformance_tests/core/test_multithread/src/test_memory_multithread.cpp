...
void perform_memory_manipulation(const ze_command_queue_handle_t cq) {
  std::thread::id thread_id = std::this_thread::get_id();
  LOG_DEBUG << "child thread spawned with ID :: " << thread_id;
  ...
  for (uint32_t i = 0; i < thread_iters; i++) {
    ...
    lzt::get_mem_alloc_properties(context, host_ptr, &memory_properties,
                                  nullptr);
    ...
    EXPECT_EQ(ZE_RESULT_SUCCESS, zeMemGetAddressRange(context, device_ptr, NULL,
                                                      &addr_range_size));
    ...
    EXPECT_EQ(ZE_RESULT_SUCCESS, zeMemGetAddressRange(context, shared_ptr, NULL,
                                                      &addr_range_size));
    EXPECT_GE(addr_range_size, alloc_size);
...
TEST(
  ...
  for (uint32_t i = 0; i < num_threads; i++) {
    threads[i] = std::make_unique<std::thread>(perform_memory_manipulation, cq);
  }
