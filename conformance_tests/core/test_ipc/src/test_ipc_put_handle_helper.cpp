...
#ifdef __linux__
static void child_put_device_test(int size, ze_ipc_memory_flags_t flags,
                                  bool is_immediate) {
...
static void child_put_subdevice_test(int size, ze_ipc_memory_flags_t flags,
                                     bool is_immediate) {
  auto driver = lzt::get_default_driver();
  auto context = lzt::create_context(driver);
  auto device = lzt::zeDevice::get_instance()->get_device();
...
int main() {
  ...
  while (true) {
    try {
      shm = bipc::shared_memory_object(bipc::open_only, "ipc_put_handle_test",
                                       bipc::read_write);
      break;
