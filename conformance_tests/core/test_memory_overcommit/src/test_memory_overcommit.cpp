...
protected:
  ...
  void run_functions(const ze_device_handle_t device, ze_module_handle_t module,
    ...
    EXPECT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmd_bundle.record_list(), fill_function, &thread_group_dimensions,
        nullptr, 0, nullptr));

    EXPECT_ZE_RESULT_SUCCESS(zeCommandListAppendBarrier(
        cmd_bundle.record_list(), nullptr, 0, nullptr));

    EXPECT_ZE_RESULT_SUCCESS(zeCommandListAppendLaunchKernel(
        cmd_bundle.record_list(), test_function, &thread_group_dimensions,
        nullptr, 0, nullptr));
