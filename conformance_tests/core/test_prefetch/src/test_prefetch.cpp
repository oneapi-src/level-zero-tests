...
protected:
  uint64_t GetPageFaultCount() {
    FILE *fp;
    char buffer[1024];
    fp = popen("sudo grep svm_pagefault_count /sys/kernel/debug/dri/0/gt0/stats | sed 's/svm_pagefault_count: //'", "r"); 
    if (fp == NULL) {
...
//    NOTE:  cannot use LOG_DEBUG or LOG_INFO because redirecting stderr to file
//
...
//           is_immediate, isCopyOnly, size, prefetch_size);
    ...
    if (prefetch_size){
        ...
        EXPECT_ZE_RESULT_SUCCESS(
           zeCommandListAppendMemoryPrefetch(cmd_bundle.list, src_memory, prefetch_size));
           expected_ioctl_count++;
        }
        if (is_dst_shared_system) {
        EXPECT_ZE_RESULT_SUCCESS(
           zeCommandListAppendMemoryPrefetch(cmd_bundle.list, dst_memory, prefetch_size));
           expected_ioctl_count++;
        }
    }
       ...
       bool dst_shared = true;
        for (uint32_t i = 0; i < 3; i++) {
            ...
            for (uint32_t j = 0 ; j < 3; j++) {
                RunAppendMemoryCopyWithPrefetch(src_shared, dst_shared, false, false, size, j*step);
    ...
    for (uint32_t i = 0; i < 3; i++) {
        ...
        for (uint32_t j = 0; j < 20; j++) {
            for (uint32_t k = 0 ; k < 3; k++) {
                RunAppendMemoryCopyWithPrefetch(src_shared, dst_shared, false, false, size, k*step);
