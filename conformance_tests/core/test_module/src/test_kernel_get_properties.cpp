...
TEST(zeKernelMaxGroupSize, GivenKernelGetMaxGroupSize) {
  ...
  auto module = lzt::create_module(context, device, "module_add.spv",
                                   ZE_MODULE_FORMAT_IL_SPIRV, "-ze-opt-large-register-file", nullptr);
  std::string kernelName = "module_add_constant";
  auto kernel = lzt::create_function(module, kernelName);

  ze_kernel_max_group_size_properties_ext_t workGroupProperties = {};
  workGroupProperties.stype = ZE_STRUCTURE_TYPE_KERNEL_MAX_GROUP_SIZE_EXT_PROPERTIES;
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeKernelGetProperties(kernel, &kernelProperties));

  LOG_INFO << "workGroupProperties.maxGroupSize = " <<  workGroupProperties.maxGroupSize;
  
  ze_device_compute_properties_t computeProperties = {};
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeDeviceGetComputeProperties(device, &computeProperties));
  
  LOG_INFO << "computeProperties.maxTotalGroupSize = " <<  computeProperties.maxTotalGroupSize;
  
  EXPECT_LE(workGroupProperties.maxGroupSize, computeProperties.maxTotalGroupSize);
