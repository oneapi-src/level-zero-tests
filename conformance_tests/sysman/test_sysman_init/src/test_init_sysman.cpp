...
TEST(SysmanInitTests,
     GivenCoreAndSysmanInitializedSysmanHandleWorksForEnumFrequencyDomanis) {
  ...
  uint32_t count = 0;
  ASSERT_EQ(ZE_RESULT_SUCCESS, zesDeviceEnumFrequencyDomains(devices[0], &count, nullptr));

}

TEST(SysmanInitTests,
     GivenCoreAndSysmanInitializedCoreHandleReturnsUninitializedErrorForEnumFrequencyDomanis) {
  static char sys_env[] = "ZES_ENABLE_SYSMAN=1";
  putenv(sys_env);
  ...
  std::vector<zes_device_handle_t> devices(device_count);
  ASSERT_EQ(ZE_RESULT_SUCCESS, zeDeviceGet(drivers[0], &device_count, devices.data()));

  uint32_t count = 0;
  ASSERT_EQ(ZE_RESULT_ERROR_UNINITIALIZED, zesDeviceEnumFrequencyDomains(devices[0], &count, nullptr));
}
