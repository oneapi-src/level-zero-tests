...
public:
  bool is_fabric_supported = false;
};
...
LZT_TEST_F(
  ...
  for (auto device : devices) {
    uint32_t count = 0;
    count = lzt::get_fabric_port_handles_count(device);
    if (count > 0) {
      is_fabric_supported = true;
      LOG_INFO << "Fabric handles are available on this device! ";
      lzt::get_fabric_port_handles(device, count);
    } else {
      LOG_INFO << "No fabric handles found for this device! ";
    }
  }
  if (!is_fabric_supported) {
    FAIL() << "No fabric handles found on any of the devices! ";
  }
}
...
LZT_TEST_F(
  ...
  for (auto device : devices) {
    ...
    actual_count = lzt::get_fabric_port_handles_count(device);
    if (actual_count > 0) {
      is_fabric_supported = true;
      LOG_INFO << "Fabric handles are available on this device! ";
