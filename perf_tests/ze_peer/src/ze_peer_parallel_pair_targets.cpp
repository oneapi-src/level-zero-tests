...
void ZePeer::perform_bidirectional_parallel_copy_to_pair_targets(
  ...
  for (auto pair_device_id : pair_device_ids) {
    ...
    auto remote_device_id = pair_device_id.second;
...
void ZePeer::perform_bidirectional_parallel_copy_to_pair_targets_immediate(
    peer_test_t test_type, peer_transfer_t transfer_type,
    std::vector<std::pair<uint32_t, uint32_t>> &pair_device_ids,
    size_t buffer_size, bool divide_buffers) {
        ...
        if (divide_buffers) {
          ...
          if (transfer_type == PEER_WRITE) {
            SUCCESS_OR_TERMINATE(zeCommandListAppendMemoryCopy(
                ze_peer_devices[local_device_id].engines[queue_index].second,
                ze_dst_buffers[remote_device_id],
                ze_src_buffers[local_device_id], buffer_size, nullptr, 1,
                &event));
            SUCCESS_OR_TERMINATE(zeCommandListAppendMemoryCopy(
                ze_peer_devices[remote_device_id].engines[queue_index].second,
                ze_dst_buffers[local_device_id],
                ze_src_buffers[remote_device_id], buffer_size, nullptr, 1,
                &event));
          } else {
            SUCCESS_OR_TERMINATE(zeCommandListAppendMemoryCopy(
                ze_peer_devices[local_device_id].engines[queue_index].second,
...
void ZePeer::perform_parallel_copy_to_pair_targets(
    peer_test_t test_type, peer_transfer_t transfer_type,
    std::vector<std::pair<uint32_t, uint32_t>> &pair_device_ids,
    size_t buffer_size, bool divide_buffers) {
...
void ZePeer::perform_parallel_copy_to_pair_targets_immediate(
    peer_test_t test_type, peer_transfer_t transfer_type,
    std::vector<std::pair<uint32_t, uint32_t>> &pair_device_ids,
    size_t buffer_size, bool divide_buffers) {
        ...
        if (divide_buffers) {
          for (size_t e = 0; e < num_engines; e++) {
            ...
            SUCCESS_OR_TERMINATE(zeCommandListAppendMemoryCopy(
                command_list,
                reinterpret_cast<void *>(
                    reinterpret_cast<uint64_t>(dst_buffer) + e * chunk),
                reinterpret_cast<void *>(
                    reinterpret_cast<uint64_t>(src_buffer) + e * chunk),
                chunk, nullptr, 1, &event));
          }
        } else {
          uint32_t queue_index =
          ...
          ze_command_list_handle_t command_list =
              ze_peer_devices[device_id].engines[queue_index].second;
          SUCCESS_OR_TERMINATE(zeCommandListAppendMemoryCopy(
              command_list, dst_buffer, src_buffer, buffer_size, nullptr, 1,
              &event));
        }
      }
...
void ZePeer::bandwidth_latency_parallel_to_pair_targets(
  ...
  std::unordered_map<uint32_t, bool> local_device_map;
  std::unordered_map<uint32_t, bool> remote_device_map;

  for (auto pair_device_id : pair_device_ids) {
    auto local_device_id = pair_device_id.first;
    auto remote_device_id = pair_device_id.second;

    if (local_device_map.find(local_device_id) == local_device_map.end()) {
      local_device_ids.push_back(local_device_id);
      local_device_map[local_device_id] = true;
    }

    if (remote_device_map.find(remote_device_id) == remote_device_map.end()) {
      remote_device_ids.push_back(remote_device_id);
      remote_device_map[remote_device_id] = true;
    }
  }
  ...
  if (bidirectional) {
    if (ZePeer::use_immediate_cmdlist) {
      perform_bidirectional_parallel_copy_to_pair_targets_immediate(
          test_type, transfer_type, pair_device_ids, buffer_size,
          divide_buffers);
    } else {
      perform_bidirectional_parallel_copy_to_pair_targets(
          test_type, transfer_type, pair_device_ids, buffer_size,
          divide_buffers);
    }
  } else {
    if (ZePeer::use_immediate_cmdlist) {
      ...
      perform_parallel_copy_to_pair_targets(test_type, transfer_type,
                                            pair_device_ids, buffer_size,
                                            divide_buffers);
    }
  }
