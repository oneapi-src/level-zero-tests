...
void ZePeak::ze_peak_sp_compute(L0Context &context) {
  ...
  std::vector<uint8_t> binary_file =
      context.load_binary_file("ze_sp_compute.spv");
  ...
  if (context.sub_device_count) {
    ...
    for (auto device : context.sub_devices) {
      result = zeMemAllocDevice(context.context, &in_device_desc, sizeof(float),
  ...
  std::vector<ze_kernel_handle_t> sp_v1;
  std::vector<ze_kernel_handle_t> sp_v2;
  std::vector<ze_kernel_handle_t> sp_v4;
  std::vector<ze_kernel_handle_t> sp_v8;
  std::vector<ze_kernel_handle_t> sp_v16;
  ...
  if (context.sub_device_count) {
    sp_v1.resize(context.sub_device_count);
    sp_v2.resize(context.sub_device_count);
    sp_v4.resize(context.sub_device_count);
    sp_v8.resize(context.sub_device_count);
    sp_v16.resize(context.sub_device_count);
    for (uint32_t i = 0; i < context.sub_device_count; i++) {
      setup_function(context, sp_v1[i], "compute_sp_v1", dev_in_val[i],
                     dev_out_buf[i]);
      setup_function(context, sp_v2[i], "compute_sp_v2", dev_in_val[i],
                     dev_out_buf[i]);
      setup_function(context, sp_v4[i], "compute_sp_v4", dev_in_val[i],
                     dev_out_buf[i]);
      setup_function(context, sp_v8[i], "compute_sp_v8", dev_in_val[i],
                     dev_out_buf[i]);
      setup_function(context, sp_v16[i], "compute_sp_v16", dev_in_val[i],
                     dev_out_buf[i]);
    }
  } else {
    ...
    setup_function(context, compute_sp_v16, "compute_sp_v16",
  ...
  if (context.sub_device_count) {
    uint32_t i = 0;
    for (auto device : context.sub_devices) {
      timed += run_kernel(context, sp_v1[i], workgroup_info, type);
      i++;
    }
    gflops =
  ...
  if (context.sub_device_count) {
    uint32_t i = 0;
    for (auto device : context.sub_devices) {
      timed += run_kernel(context, sp_v2[i], workgroup_info, type);
      i++;
    }
    gflops =
        calculate_gbps(timed, number_of_work_items * context.sub_device_count *
  ...
  if (context.sub_device_count) {
    uint32_t i = 0;
    for (auto device : context.sub_devices) {
      timed += run_kernel(context, sp_v4[i], workgroup_info, type);
      i++;
    }
    gflops =
        calculate_gbps(timed, number_of_work_items * context.sub_device_count *
  ...
  if (context.sub_device_count) {
    uint32_t i = 0;
    for (auto device : context.sub_devices) {
      timed += run_kernel(context, sp_v8[i], workgroup_info, type);
      i++;
    }
    gflops =
        calculate_gbps(timed, number_of_work_items * context.sub_device_count *
  ...
  if (context.sub_device_count) {
    uint32_t i = 0;
    for (auto device : context.sub_devices) {
      timed += run_kernel(context, sp_v16[i], workgroup_info, type);
      i++;
    }
    gflops =
        calculate_gbps(timed, number_of_work_items * context.sub_device_count *
  ...
  if (context.sub_device_count) {
    ...
    result = zeKernelDestroy(compute_sp_v2);
    if (result) {
      throw std::runtime_error("zeKernelDestroy failed: " +
                               std::to_string(result));
    }
  ...
  if (context.sub_device_count) {
    ...
    result = zeKernelDestroy(compute_sp_v4);
    if (result) {
      throw std::runtime_error("zeKernelDestroy failed: " +
                               std::to_string(result));
    }
  ...
  if (context.sub_device_count) {
    ...
    result = zeKernelDestroy(compute_sp_v8);
    if (result) {
      throw std::runtime_error("zeKernelDestroy failed: " +
                               std::to_string(result));
    }
  ...
  if (context.sub_device_count) {
    ...
    result = zeKernelDestroy(compute_sp_v16);
    if (result) {
      throw std::runtime_error("zeKernelDestroy failed: " +
                               std::to_string(result));
    }
